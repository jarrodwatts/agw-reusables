...
export default function ProfileDataDemo() {
  ...
  const { data: specificProfile } = useAbstractProfileByAddress("0x1C67724aCc76821C8aD1f1F87BA2751631BAbD0c")
    ...
    <div className="space-y-6">
      ...
      <div className="border rounded-lg p-4">
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              <div 
                ...
                style={{ borderColor: getTierColor(profile.user.tier) }}
              />
              <div>
                <p className="font-medium">{profile.user.name}</p>
                <p className="text-sm text-muted-foreground">
                  {getTierName(profile.user.tier)} Tier • {profile.user.badges.length} badges
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-2 text-xs">
              <span className={profile.user.hasStreamingAccess ? "text-green-600" : "text-gray-500"}>
                {profile.user.hasStreamingAccess ? "✓" : "✗"} Streaming Access
              </span>
              <span className={profile.user.hasCompletedWelcomeTour ? "text-green-600" : "text-gray-500"}>
                {profile.user.hasCompletedWelcomeTour ? "✓" : "✗"} Welcome Tour
              </span>
            </div>
            ...
            {profile.user.badges.length > 0 && (
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1">
                  {profile.user.badges.slice(0, 3).map((badge, index) => (
                    <span 
                      key={index}
                  ...
                  {profile.user.badges.length > 3 && (
                    <span className="text-xs text-muted-foreground">
                      +{profile.user.badges.length - 3} more
                    </span>
                  )}
                </div>
              </div>
          ...
          <p className="text-muted-foreground">No profile found for connected wallet</p>
        )}
      </div>
      ...
      {specificProfile && (
        <div className="border rounded-lg p-4">
          <h3 className="font-semibold mb-3">Specific Address Profile</h3>
          <div className="flex items-center gap-3">
            <div 
              className="w-6 h-6 rounded-full border-2" 
              style={{ borderColor: getTierColor(specificProfile.user.tier) }}
            />
            <div>
              <p className="font-medium">{specificProfile.user.name}</p>
              <p className="text-sm text-muted-foreground">
                {getTierName(specificProfile.user.tier)} • {specificProfile.user.verification}
              </p>
            </div>
          </div>
        </div>
