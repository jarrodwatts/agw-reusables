...
```
  ...
  <Step step={3} title="Required: Install the connect wallet button">
  If you haven't already, install the [connect wallet button](/docs/authentication/connect-wallet-button) component:
  ...
  <Step step={4} title="Optional: Install SIWE authentication">
  To require users to authenticate with a [SIWE Button](/docs/authentication/siwe-button), install the `siwe-button` component:
  ...
  <Step step={5} title="Optional: Install session keys">
  To require users to create a [session key](/docs/session-keys/create-session-key-button), install the `session-keys` component:
